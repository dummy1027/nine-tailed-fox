...
function PrivateBattle() {
  const [roomCode, setRoomCode] = useState(''); // 생성된 방 코드 저장
  const [isWaiting, setIsWaiting] = useState(false); // 대기실 화면 전환 상태

  // "새로운 방 만들기" 버튼 클릭 시 실행
  const handleCreateRoom = () => {
    ...
    setRoomCode(newCode); // 6자리 코드 생성
    setIsWaiting(true);   // 대기실로 화면 전환
  };

  const handleJoinRoom = (e) => {
    e.preventDefault();
    const inputCode = e.target.roomCode.value.trim().toUpperCase();
  ...
  if (isWaiting) {
    return (
      <div className="container" style={{ padding: '100px 20px', textAlign: 'center' }}>
        <div style={{ backgroundColor: 'var(--theme-surface)', padding: '50px', borderRadius: '24px', maxWidth: '500px', margin: '0 auto', border: '1px solid var(--theme-border)' }}>
          <h2 style={{ color: 'var(--theme-text)' }}>전투 대기실</h2>
          <p style={{ color: 'var(--theme-secondary-text)' }}>상대방에게 아래 코드를 알려주세요</p>
          <div style={{ margin: '30px 0', padding: '20px', background: '#fff', border: '2px dashed var(--tesla-blue)', borderRadius: '12px' }}>
            <span style={{ fontSize: '32px', fontWeight: '800', letterSpacing: '5px', color: 'var(--tesla-blue)' }}>{roomCode}</span>
          </div>
          <button className="paradox-button" onClick={() => setIsWaiting(false)} style={{ background: '#ff4b4b' }}>취소하고 나가기</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container" style={{ padding: '100px 20px', textAlign: 'center' }}>
      <h1 className="text-gradient" style={{ fontSize: '50px', marginBottom: '40px' }}>비공개 배틀</h1>
      <div style={{ display: 'flex', gap: '30px', justifyContent: 'center' }}>
        {/* 방 만들기 섹션 */}
        <div style={{ backgroundColor: 'var(--theme-surface)', padding: '40px', borderRadius: '24px', width: '300px', border: '1px solid var(--theme-border)' }}>
          <div style={{ fontSize: '40px', marginBottom: '20px' }}>🏰</div>
          <h3 style={{ color: 'var(--theme-text)' }}>새로운 방 만들기</h3>
          <p style={{ color: 'var(--theme-secondary-text)', fontSize: '14px', marginBottom: '30px' }}>친구가 접속할 수 있는<br/>고유 코드를 생성합니다.</p>
          <button className="btn paradox-bg" onClick={handleCreateRoom} style={{ width: '100%' }}>방 생성하기</button>
        </div>

        {/* 방 참가하기 섹션 */}
        <div style={{ backgroundColor: 'var(--theme-surface)', padding: '40px', borderRadius: '24px', width: '300px', border: '1px solid var(--theme-border)' }}>
          <div style={{ fontSize: '40px', marginBottom: '20px' }}>🔑</div>
          <h3 style={{ color: 'var(--theme-text)' }}>코드로 참가하기</h3>
          <p style={{ color: 'var(--theme-secondary-text)', fontSize: '14px', marginBottom: '20px' }}>전달받은 6자리 코드를<br/>입력해 주세요.</p>
          <form onSubmit={handleJoinRoom}>
            <input name="roomCode" placeholder="CODE6" maxLength={6} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid var(--theme-border)', marginBottom: '10px', textAlign: 'center' }} />
            <button className="paradox-button" style={{ width: '100%' }}>입장하기</button>
          </form>
        </div>
      </div>
    </div>
