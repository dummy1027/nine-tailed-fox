...
export default function BattleArena() {
  ...
  /* 🚨 [버그 수리] 실시간 데미지 전송 채널 동기화 강화 */
  const subscribeToRoom = (roomId) => {
    if (channelRef.current) supabase.removeChannel(channelRef.current);

    const ch = supabase.channel(`battle_${roomId}`)
      .on('broadcast', { event: 'damage' }, (payload) => {
        console.log("💥 실시간 데미지 브로드캐스트 감지:", payload);
        const { targetRole, nextHp } = payload.payload;

        // 전달받은 타겟 역할이 내 역할과 일치하는 경우 내 피를 깎음
    ...
    ch.subscribe((status) => {
      if (status === 'SUBSCRIBED') {
        console.log("✅ 실시간 배틀 브로드캐스트 구독 활성화!");
      }
    });
  ...
  /* 🚨 [버그 수리] 문제 정답 시 상대방 최종 HP 상태값 명시 및 브로드캐스트 전송 */
  const handleCheck = async () => {
    ...
    if (correct) {
      ...
      // 상대방 브라우저 화면의 HP도 정확하게 동기화하기 위해 타겟 역할군과 수치를 명확히 지정해 발송
      if (channelRef.current) {
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div style={{ background: 'var(--theme-surface, #1a1d27)', borderRadius: 14, border: '1px solid var(--theme-border, #2a2d3a)', display: 'flex', flexDirection: 'column' }}>
            ...
            {/* 채점(check)일 때는 한가운데 정렬, 코드 실행(run)일 때는 왼쪽 위부터 타이트하게 채움 */}
            <div style={{ 
              flex: 1, 
              padding: 14, 
              overflow: 'auto',
              ...
              justifyContent: resultType === 'check' ? 'center' : 'flex-start',
              alignItems: resultType === 'check' ? 'center' : 'stretch'
            }}>
              {/* ▶ 일반 코드 실행 결과 창 (왼쪽 정렬 레이아웃 적용) */}
              {resultType === 'run' && output && (
                <div style={{ padding: 12, borderRadius: 10, background: 'rgba(0,0,0,0.3)', border: '1px solid var(--theme-border, #2a2d3a)', width: '100%' }}>
                  <p style={{ 
                    color: output.startsWith('[E') ? '#f87171' : '#4ade80', 
              ...
              {/* ✓ 정답 / 오답 제출 판정 창 (가운데 정렬 팝업 디자인 유지) */}
              {resultType === 'check' && isCorrect !== null && (
                <div style={{
                  ...
                  <p style={{ fontWeight: 800, fontSize: 18, color: isCorrect ? '#2ecc71' : '#e74c3c', fontFamily: "'Nunito', sans-serif", margin: 0 }}>
                    {isCorrect ? '정답! -' + DAMAGE_PER_CORRECT + ' 데미지!' : '오답! 다시 시도하세요.'}
                  </p>
                  {isCorrect && <p style={{ color: '#8b8fa3', fontSize: 13, marginTop: 6, marginBottom: 0 }}>다음 문제가 곧 나옵니다...</p>}
                  {!isCorrect && output && (
                    <p style={{ color: '#f87171', fontFamily: 'monospace', fontSize: 13, marginTop: 8, marginBottom: 0, textAlign: 'left' }}>{output}</p>
                  )}
                </div>
              )}
              
              {/* 아무것도 안 했을 때의 디폴트 가이드 (가운데 배치) */}
              {!resultType && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#6b7280', fontSize: 14, alignSelf: 'center' }}>
